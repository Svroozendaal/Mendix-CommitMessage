...
internal static class AutoCommitMessageExportService
{
    ...
    private static ExportModelChange[] ToExportModelChanges(IReadOnlyList<MendixModelChange>? modelChanges)
    {
        ...
        return modelChanges
            .Select(modelChange => new ExportModelChange(
                ...
                modelChange.Details,
                MendixModelChangeDisplayTextFormatter.Format(modelChange)))
            .ToArray();
    }
    ...
    private static string ResolveBranchName(AutoCommitMessagePayload payload, Repository repository)
    {
